...
import { type DataFormat, SplitButton } from "@/components/SplitButton"
import { filterHeaders, groupResponseHeaders } from "@/lib/headers"
import { type CdnPreset, detectPreset } from "@/lib/presets"
...
export function ResponsePanel({
  ...
  const detected = useMemo(() => detectPreset(headers.map((h) => h.name)), [headers])
  ...
  return (
    <div className='flex min-h-0 flex-1 flex-col overflow-y-auto'>
      ...
      {hops.length > 1 ? <RedirectChain hops={hops} selected={selected} onSelect={setHop} /> : null}
      ...
      {skin === "classic" ? (
        <p className='skin-sm border-b border-line bg-warn-soft px-3 py-1.5 text-ink-dim'>
          The first 5 header values are taken from the X-Cache, X-Cache-Key and X-Check-Cacheable
          values. (display purpose only)
        </p>
      ) : null}

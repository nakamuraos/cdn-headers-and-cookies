...
export function registerCapture(): void {
      ...
      enqueue(details.tabId, async () => {
        ...
        const hop: RequestHop = {
          ...
          requestHeaders: truncateHeaders(
            withInjected(toEntries(details.requestHeaders), injectedHeaders(settings, host)),
          ),
          responseHeaders: [],
        }
        ...
        const existing = (await logFor(details.tabId)).find((r) => r.id === details.requestId)
  ...
  browser.webRequest.onHeadersReceived.addListener(
    (details) => {
      ...
      enqueue(details.tabId, async () => {
        ...
        await rememberCdn(hostFromUrl(details.url), toEntries(details.responseHeaders))
      })
    },
    URL_FILTER,
  ...
  browser.webRequest.onCompleted.addListener((details) => {
    if (details.tabId < 0) return
    enqueue(details.tabId, () => update(details.tabId, details.requestId, { completed: true }))
  }, URL_FILTER)

...
export function RequestPanel({
  ...
  return (
    <div className='flex min-h-0 flex-1 flex-col overflow-y-auto'>
      ...
      <HeaderTable headers={rows} preset={preset} sort={sort} onSort={toggleSort} />
      ...
      <div className='mt-auto shrink-0 border-t border-line bg-surface-2'>
        <div className='flex items-center justify-between gap-2 border-b border-line px-3 py-2'>
          ...
          <Button
            variant='primary'
            onClick={() =>
              onCustomHeadersChange([...customHeaders, { name: "", value: "", enabled: true }])
            }
          >
            Add header
          </Button>
        ...
        {customHeaders.length === 0 ? (
          <p className='skin-sm px-3 py-2.5 text-ink-dim'>No custom headers for this host yet.</p>
        ) : (
          customHeaders.map((header, index) => (
            <CustomHeaderRow
              ...
              onDelete={() => onCustomHeadersChange(customHeaders.filter((_, i) => i !== index))}
            />
          ))
        )}
      </div>
